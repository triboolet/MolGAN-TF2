...
def reward(A, X, datas, metric='validity') :
    metric = 'validity,novelty'
    mols = [datas.matrices2mol(n_, e_, strict=True) for n_, e_ in zip(X, A)]
    rr = 1.
    for m in ('logp,sas,qed,unique' if metric == 'all' else metric).split(','):
        if m == 'np':
            ...
            rr *= MolecularMetrics.novel_scores(mols, datas)
        elif m == 'dc':
            rr *= MolecularMetrics.drugcandidate_scores(mols, datas)
        elif m == 'unique':
            rr *= MolecularMetrics.unique_scores(mols)
        elif m == 'diversity':
            rr *= MolecularMetrics.diversity_scores(mols, datas)
        elif m == 'validity':
